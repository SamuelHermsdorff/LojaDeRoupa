...
async function confirmarAcao(confirmado) {
    ...
    if (action === "deletar" && id) {
        try {
            const response = await fetch(`../PHP/controle_clientes.php?action=deletar`, {
                method: "POST",
                headers: {
...
function listarUsuarios() {
    ...
    fetch(`../PHP/controle_clientes.php?action=listar&search=${encodeURIComponent(searchTerm)}`)
        .then(res => res.json())
        .then(usuarios => {
            ...
            usuarios.forEach(usuario => {
                ...
                if (parseInt(usuario.funcionario) == 1 || parseInt(usuario.administrador) == 1) return;
                row.innerHTML = `
                    <td>${usuario.id}</td>
                    <td>${usuario.nome}</td>
                    <td>${usuario.cpf}</td>
...
function abrirModalEditar(id) {
    ...
    fetch(`../PHP/controle_clientes.php?action=buscar&id=${id}`)
        .then(res => res.json())
        .then(usuario => {
            if (usuario.erro) {
...
async function verificarEmailExistente(email, idUsuario = null) {
    try {
        ...
        const url = `../PHP/controle_clientes.php?action=verificar_email&email=${encodeURIComponent(email)}${idUsuario ? `&id=${idUsuario}` : ''}`;
...
async function salvarUsuario(event) {
    ...
    try {
        ...
        // Enviar dados
        const response = await fetch(`../PHP/controle_clientes.php?action=${action}`, {
            method: "POST",
            body: formData
        });
...
async function verificarEmailExistente(email, id = null) {
    try {
        const url = `../PHP/controle_clientes.php?action=verificar_email&email=${encodeURIComponent(email)}${id ? `&id=${id}` : ''}`;
        const response = await fetch(url);
        const data = await response.json();
