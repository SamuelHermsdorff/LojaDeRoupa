...
function buscarUsuario($conn) {
    ...
    $stmt = $conn->prepare("SELECT id, nome, cpf, telefone, email, funcionario, administrador FROM Usuarios WHERE id = ?");
...
function cadastrarUsuario($conn) {
    ...
    $administrador = isset($_POST['administrador']) && $_POST['administrador'] === '1' ? 1 : 0;
    ...
    if ($action === 'editar') {
        ...
        $stmt = $conn->prepare("INSERT INTO Usuarios (nome, cpf, telefone, email, senha, funcionario, administrador) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssii", $nome, $cpf, $telefone, $email, $senha, $funcionario, $administrador);
    }
...
function editarUsuario($conn) {
    ...
    try {
        ...
        $sql = "UPDATE Usuarios SET nome = ?, cpf = ?, telefone = ?, email = ?, funcionario = ?, administrador = ?";
        $types = "ssssii";
        $params = [
            $dados['nome'],
            $dados['cpf'],
